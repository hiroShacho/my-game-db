  ...
  return (
    <>
      ...
      <div className="mx-auto max-w-3xl px-2 sm:px-4 py-6">
        ...
        {/* 各ロールの特殊効果について */}
        <SectionTitle icon="auto_fix_high">各ロールの特殊効果について</SectionTitle>
        <div className="mb-4">
          武器を３つ装備することで発動する武器共鳴効果にはそれぞれソロ・チームで発動するバフが設定されている。<br />
          <b>強攻</b>であればソロなら最終ダメージ+10%、チームなら40%追加の最終ダメージ+50%のバフが発動する。<br />
          <b>剛毅</b>であればソロならダメージ軽減+25%・シールドブレイク率+60%・ヘイト+800%・攻撃を当てた武器属性の弱点付与効果、チームなら20%追加のダメージ軽減+45%のバフが発動する。<br />
        ...
        <CaptionedImage
          src="/Newbie/basicknowledge/basicknowledge_roll.PNG"
          alt="各ロールごとにバフが発動"
          caption="各ロールごとにバフが発動"
        ...
        <div className="mb-4">
          ...
          ソロだと時々恩恵編成のサボテン火力が強攻編成よりも高いことがあるが、チームでは強攻に最終ダメージが追加で40%付く上に恩恵が使うフィオナのリンク（味方を1人指定してバフを付与する効果）による攻撃力UPは強攻・均衡に対してしか効果が無い。<br />
          そのため、チームでの戦闘では余程のことが無い限り火力恩恵が強攻に勝つことは不可能となっている。
        </div>
        <CaptionedImage
          src="/Newbie/basicknowledge/basicknowledge_fionalink.PNG"
          alt="フィオナのリンクは強攻・均衡にしか攻撃力を渡せない"
          caption="フィオナのリンクは強攻・均衡にしか攻撃力を渡せない"
          onClick={() => openModal("/Newbie/basicknowledge/basicknowledge_fionalink.PNG", "フィオナのリンクは強攻・均衡にしか攻撃力を渡せない")}
        />

        {/* 属性は1つに絞ろう（メイン属性を決めよう） */}
        <SectionTitle icon="bolt">属性は1つに絞ろう（メイン属性を決めよう）</SectionTitle>
        <div className="mb-4">
          このゲームはのガチャは武器なら80連・ボリションなら40連でSSR確定の天井が存在するが、SSRの中からピックアップキャラの武器・ボリションが出る確率は50%となっている。<br />
        ...
        <CaptionedImage
          src="/Newbie/basicknowledge/basicknowledge_elemnt.PNG"
          alt="自分にあった属性を1つ選ぼう！"
          caption="自分にあった属性を1つ選ぼう！"
          onClick={() => openModal("/Newbie/basicknowledge/basicknowledge_elemnt.PNG", "自分にあった属性を1つ選ぼう！")}
        />

        {/* ガチャはメイン属性の最新キャラを引こう！ */}
        <SectionTitle icon="star">ガチャはメイン属性の最新キャラを引こう！</SectionTitle>
        <div className="mb-4">
        ...
        <CaptionedImage
          src="/Newbie/basicknowledge/basicknowledge_newchara.PNG"
          alt="最新キャラを引けばほぼ確実に強くなれるという意味では安心？"
          caption="最新キャラを引けばほぼ確実に強くなれるという意味では安心？"
          onClick={() => openModal("/Newbie/basicknowledge/basicknowledge_newchara.PNG", "最新キャラを引けばほぼ確実に強くなれるという意味では安心？")}
        />

        {/* 限定ガチャの天井は全て共有 */}
        <SectionTitle icon="diamond">限定ガチャの天井は全て共有</SectionTitle>
        <div className="mb-4">
        ...
        <CaptionedImage
          src="/Newbie/basicknowledge/basicknowledge_pity.PNG"
          alt="限定ガチャの天井はバージョンをまたいで共有されるぞ！"
          caption="限定ガチャの天井はバージョンをまたいで共有されるぞ！"
          onClick={() => openModal("/Newbie/basicknowledge/basicknowledge_pity.PNG", "限定ガチャの天井はバージョンをまたいで共有される")}
        />

        {/* 新キャラクターの属性の実装順はほぼ固定 */}
        <SectionTitle icon="timeline">新キャラクターの属性の実装順はほぼ固定</SectionTitle>
        <div className="mb-4">
        ...
        <CaptionedImage
          src="/Newbie/basicknowledge/basicknowledge_gachaelement.PNG"
          alt="実装は氷⇒物理⇒雷⇒炎⇒氷...とループする（時々異能が挟まる）"
          caption="実装は氷⇒物理⇒雷⇒炎⇒氷...とループする（時々異能が挟まる）"
          onClick={() => openModal("/Newbie/basicknowledge/basicknowledge_gachaelement.PNG", "実装は氷⇒物理⇒雷⇒炎⇒氷...とループする（時々異能が挟まる）")}
        />

        {/* デイリーなどの更新は午前5時 */}
        <SectionTitle icon="schedule">デイリーなどの更新は午前5時</SectionTitle>
        <div className="mb-4">
        ...
        <CaptionedImage
          src="/Newbie/basicknowledge/basicknowledge_dailyupdate.PNG"
          alt="異空間狩猟場だけ特殊なので注意！"
          caption="異空間狩猟場だけ特殊なので注意！"
          onClick={() => openModal("/Newbie/basicknowledge/basicknowledge_dailyupdate.PNG", "異空間狩猟場だけ特殊なので注意！")}
        />

        {/* ギルドは入り得 */}
        <SectionTitle icon="groups">ギルドは入り得</SectionTitle>
        <div className="mb-4">
          ギルドに入ると毎週煙水晶が貰える。<br />
        ...
        <CaptionedImage
          src="/Newbie/basicknowledge/basicknowledge_crewrewards.PNG"
