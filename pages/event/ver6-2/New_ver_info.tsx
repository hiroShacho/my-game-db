...
const rerunWeaponIds = [
  "w_70", "w_65", "w_61","w_56", "w_68", "w_72", "w_67", "w_63", "w_71", "w_66", "w_62", "w_69", "w_64",
];
// 対象武器データ取得
...
const costumeImages = [
  {
    src: "/ver_event/Outfit_SummerEncounter.PNG",
    alt: "完全網羅データ：「夏のときめき」",
    label: "完全網羅データ：「夏のときめき」",
    date: "08/04(火)メンテナンス後 -  08/25(火)06:00（JST）",
  },
  {
    src: "/ver_event/Simulacrum_Gefion.PNG",
    alt: "プレアバ：ゲルフィン",
    label: "プレアバ：ゲルフィン",
    date: "08/04(火)メンテナンス後 -  09/15(火)06:00（JST）",
  }
];
...
const gachaDates = [
  ...
  {
    label: "【復刻】ガチャマシン：「オリジナル・サマー」",
    date: "08/04(火)メンテナンス後 -  08/25(火)06:00（JST）",
  },
  {
    label: "【復刻】ガチャマシン：「海塩スターコーデ」",
    date: "08/25(火)13:00（JST） -  09/15(火)06:00（JST）",
  },
  {
    label: "【復刻】コスチュームガチャ",
    date: "08/25(火)13:00（JST） -  09/15(火)06:00（JST）",
  ...
  {
    label: "プレミアムアバター：ゲルフィン",
    date: "08/04(火)メンテナンス後 -  09/15(火)06:00（JST）",
  },
  {
    label: "【復刻】プレミアムアバターセット：すべて",
    date: "08/04(火)メンテナンス後 -  08/25(火)06:00（JST）",
  },
  {
    label: "【復刻】プレミアムアバターセット：氷",
    date: "08/25(火)13:00（JST） -  09/15(火)06:00（JST）",
  },
];
...
export default function NewVerInfo() {
  ...
  return (
    <>
      <Head>
        <title>ver6.2アップデート情報まとめ | 幻塔攻略データベース</title>
        <meta name="viewport" content="width=device-width, initial-scale=1.0" />
        <meta name="description" content="幻塔 ver6.2 新アップデート内容まとめページ" />
      </Head>
      <div className="w-full max-w-full min-h-screen space-y-8 px-2 sm:px-6 py-4">
        ...
        {/* 新キャラクター・ゲルフィン実装！ */}
        <section>
          <SectionTitle icon={<><span title="氷">🧊</span><span title="ゲルフィン">🎪</span></>}>
            新キャラクター・ゲルフィン実装！
          </SectionTitle>
          ...
          <div className="flex gap-4 flex-wrap mb-4">
            ...
            >
              <div className="flex flex-col items-center gap-2">
                <Image src="/images/w_74_img.PNG" alt="ゲルフィン武器" width={48} height={48} className="rounded border" />
                <span className="font-semibold text-sky-600">武器：フェアリーリング</span>
            ...
            >
              <div className="flex flex-col items-center gap-2">
                <Image src="/images/m_70_img.PNG" alt="ゲルフィンボリション" width={48} height={48} className="rounded border" />
                <span className="font-semibold text-sky-600">ボリション：反転する座標</span>
                <div className="text-xs mt-1 text-gray-600">詳細ページへ</div>
              </div>
            </Link>
          </div>
        ...
        <section>
          ...
          <a
            href="https://tof.perfectworld.com/jp/news/notice/20260730/263354.shtml"
            target="_blank"
