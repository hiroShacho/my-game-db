...
const weapon = {
  ...
  name: "カロン",
  avatar: "ベリー",
  description: "神話において、カロンは現世を離れたばかりの亡魂を冥河の対岸へと渡す船頭である--べりーがランプを手に取り、枯れ果てた「ティトノス」たちの苦痛を和らげるたび、彼女の心の中ではそっとこんな言葉が繰り返される。「安心して、私がカロンのようにあなたたちに安らぎを与えてあげる。どれだけ時間がかかろうともね」",
  strategy: "最適編成では連携スキルとスキルを撃つだけでOK。\n連携スキルが一番火力が高いが、通常攻撃扱いのダメージなので旧クローディアボリションで火力を出すことは出来ないようになっている。",
  ratingScore: 3.5,
  ratingAttacker: 2.5,
  ratingSupporter: 3.5,
  ratingReroll: 3,
  ratingExplore: 3,
  ratingText: "物理・強攻のサブアタッカー。\n自身の連携エナジーが21秒経過で自動で溜まるようになっており、逆にそれ以外の方法ではエナジーを溜められない仕様になっている。\n連携スキルとスキルはともに特殊な効果がほぼ無いシンプルな攻撃になっている。回避長押しでランタンを投げて味方を引き寄せたり、自身をランタンの元に引き寄せたりできる移動性能も持っている。",
  videoUrls: ["https://youtu.be/2DTy18o-Juw"],
  ...
  constellations: [
    {
      level: "1凸",
      description: "物理属性ダメージ+9%。\nスキル-無痛告別ガイドで攻撃するたびに自身の最大HPの5%を回復する。",
      tags: ["物理属性ダメージUP","回復"]
    },
    {
      level: "2凸",
    ...
    {
      level: "3凸",
      description: "物理・炎耐性を25%無視する（無視する属性耐性の合計値はそれぞれ50%を超えない）。\nスキル-無痛告知ガイドのダメージ倍率が9,000%から16,500%に増加。",
      tags: ["全属性耐性無視","耐性無視"]
    },
    {
      level: "4凸",
    ...
    {
      level: "5凸",
      description: "物理属性ダメージ+9%。\n致命的なダメージを受けた時、死神の受け取り拒否（死亡を1回防ぎ、短時間ダメージを無効化。この効果は60秒ごとに最大1回発動。）を発動する。",
      tags: ["物理属性ダメージUP","死亡回避"]
    },
    {
      level: "6凸",
      description: "最終ダメージ+30%\n連携スキル-最後の挨拶の通常攻撃ダメージ部分が16,500%から25,500%に増加。",
      tags: ["最終ダメージUP"]
    }
  ]
};
...
const matrix = {
  ...
  name: "自然の意志",
  avatar: "ベリー",
  description: "ベリーのボリション（テストサーバー）",
  effects: [
    {
      set: "2セット効果",
      effect: "物理属性攻撃力+28%/30%/32%/34%。\n武器-カロンの連携スキル攻撃を受けたモンスターは、死亡時に広範囲の爆発を引き起こし、攻撃力×6,000%のダメージを与える。装備すると常時有効。複数セット装着している場合は★ランクが一番高いセットのみ有効。",
      tags: ["物理属性攻撃力UP","常時有効"]
    },
    {
      set: "4セット効果",
      effect: "物理属性ダメージ+24%/28%/32%/36、武器-カロンをセットしている時、追加で最終ダメージ+15%。装備すると常時有効。複数セット装着している場合は★ランクが一番高いセットのみ有効。",
      tags: ["物理属性ダメージUP","最終ダメージUP","常時有効"]
    }
  ],
  ratingComment: "2セット効果はおまけ程度の効果だが、どちらの効果もバフは十分強力。",
  ratingStars: "4"
};
...
const trait = {
  ...
  name: "スカルマーブル",
  avatar: "ベリー",
  description: "最終ダメージ+18%。\n回避-カロンの順風船と回避-カロンの逆風船のクールダウンをキャンセルする。",
  rating: "おもちや",
  tags: ["最終ダメージUP"]
};
  ...
  return (
    <>
      <Head>
        ...
        <meta name="description" content="幻塔（Tower of Fantasy）Ver5.8新武器・ボリション・アバター特性のテストサーバー先行情報まとめページ。" />
      </Head>
      <SidebarLayout>
        <div className="max-w-3xl mx-auto space-y-8 px-2 sm:px-6 py-6">
          <h1 className="text-2xl sm:text-3xl font-bold mb-6">Ver5.8 新武器・ボリション・アバター特性まとめ</h1>
