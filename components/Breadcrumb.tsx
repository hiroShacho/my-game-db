...
const labelMap: Record<string, string> = {
  ...
  "Lockhart_EverspringIsland":"ロックハート：ベルナ島",
  "weapons": "武器一覧",
  "matrices": "ボリション一覧",
  "trait": "アバター特性一覧",
  "relics": "アルケー一覧",
...
// ルート配列を受け取り、ラベルを返す関数
function getBreadcrumbLabel(part: string, parentPath: string[], querySlug?: string): string {
  // 動的ルーティング（[slug].tsx）などでslugを取得した場合はそちらを優先
  if (
    parentPath.length >= 1 &&
    ["weapons", "matrices", "trait", "relics"].includes(parentPath[0])
  ) {
    // slug（武器IDなど）がURLパラメータに含まれている場合は日本語名辞書を優先
    if (querySlug && weaponNameMap[querySlug]) {
      return weaponNameMap[querySlug];
    }
    // slugがない場合はURLパート（part）の日本語名変換
...
export default function Breadcrumb() {
  ...
  // URLの動的パラメータ（slugなど）を取得
  const querySlug =
    typeof router.query.slug === "string" ? router.query.slug : undefined;

  // SSR/CSR両方で安定するように router.asPath.split("?")[0].split("/") を使いつつ、[slug] は querySlug で上書き
  // ただし、Next.jsのrouter.pathnameだと[slug]になってしまうので、asPathから実際の値取得
  const pathParts = (() => {
    const asPathParts = router.asPath.split("?")[0].split("/").filter(Boolean);
    // weapons/[slug] などの場合、asPathPartsでslug値を取得できる
    if (
      querySlug &&
      asPathParts.length > 1 &&
      ["weapons", "matrices", "trait", "relics"].includes(asPathParts[0])
    ) {
      // [slug] 部分を querySlug で上書き
      asPathParts[1] = querySlug;
    }
    return asPathParts;
  })();
        ...
        <ol className="flex flex-wrap items-center space-x-1">
          {displayLabels.map((crumb, idx) => (
            <li key={crumb.href} className="flex items-center">
              ...
              <span>{getBreadcrumbLabel(crumb.part, crumb.parentPath, querySlug)}</span>
            </li>
          ))}
        </ol>
      </nav>
        ...
        {displayLabels.map((crumb, idx) => (
          <li key={crumb.href} className="flex items-center">
            ...
            <Link href={crumb.href} className="hover:underline text-blue-600">
              {getBreadcrumbLabel(crumb.part, crumb.parentPath, querySlug)}
            </Link>
          </li>
        ))}
      </ol>
